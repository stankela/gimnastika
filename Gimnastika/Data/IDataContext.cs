...
using Gimnastika.Data.QueryModel;
...
namespace Gimnastika.Data
{
    ...
    {
        /// <summary>
        /// Reports whether this <c>IDataContext</c> contain any changes which must be synchronized with the database
        /// </summary>
        bool IsDirty { get; }

        /// <summary>
        /// Reports whether this <c>IDataContext</c> is working transactionally
        /// </summary>
        ...
        IList<T> GetByCriteriaDistinct<T>(Query query) where T : class, new();
        ...
        bool SupportsQueryLanguage(QueryLanguageType type);

        IList<T> ExecuteQuery<T>(QueryLanguageType type, string query,
            string[] paramNames, object[] paramValues) where T : class, new();

        // Vraca listu proizvoljnih objekata
        IList ExecuteQuery(QueryLanguageType type, string query,
            string[] paramNames, object[] paramValues);

        IList<T> ExecuteNamedQuery<T>(string queryName,
            string[] paramNames, object[] paramValues) where T : class, new();

        // Vraca listu proizvoljnih objekata
        IList ExecuteNamedQuery(string queryName,
            string[] paramNames, object[] paramValues);
