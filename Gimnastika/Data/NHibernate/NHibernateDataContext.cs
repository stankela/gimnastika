...
namespace Gimnastika.Data.NHibernate
{
    public class NHibernateDataContext : IDataContext, IDisposable
    {
        ...
        public IList<T> GetByCriteriaDistinct<T>(Query query) where T : class, new()
        {
        ...
        public IList<T> ExecuteQuery<T>(QueryLanguageType type, string query,
        ...
        public IList<T> ExecuteNamedQuery<T>(string queryName,
        string[] paramNames, object[] paramValues) where T : class, new()
        {
            IQuery q = session.GetNamedQuery(queryName);
            for (int i = 0; i < paramNames.Length; i++)
            {
                q.SetParameter(paramNames[i], paramValues[i]);
            }
